...
/**
* @class StockData
* @brief A class responsible for processing a single stock price and dates for those stock prices.
* This class will 
* <ul>
* <li> Validate the stock date.
* <li> Validate the stock price.
* <li> Indicate whether a stock price set has bad entry.
* <li> Provide a means to compare two stock price with respect to date.
* </ul>
*/
class StockData {
public:
  ...
  * @param [in] other The other StockData object to compare with this current StockData object.
  */
...
private:
  ...
  float m_closePrice;  ///< The closing price of the stock price at the given date
  uint64_t m_dateNum;  ///< The interger format of date, suitable for handy comparisons
  bool m_hasError;     ///< A flag to indicate is the give stock data is invalid
};

/**
* @class StockPrices
* @brief A class responsible for managing stock prices and dates for those stock prices.
* This class will
* <ul>
* <li> Download the historical stock prices from csv file.
* <li> Normalize the stock prices in [0,1] range.
* <li> Re-shape and split the stock dataset into test and validation set.
* </ul>
*/
 ...
 */
  ...
  * @param [in] stockSymbol The BSE stock symbol for which historical stock prices needs to be loaded.
  * @param [in] maxDataSize The maximum number of stock prices to take into account.
  */
  virtual bool loadTimeSeries(const std::string &stockSymbol, const int64_t& maxDataSize = NetworkConstants::kMaxStockPrices) override;
  /**
  * @brief Method to split and reshape stock data
  * @param [in] testSplitRatio The ratio used for training and test dataset.
  * @param [in] num_prev_samples The number of past sample used for creation of test and training dataset.
  */
...
private:
  ...
  std::vector<float> normalizedStockClosePrices; ///< The vector of normalized stock close price
  ...
  std::vector<float> x_train;                    ///< The vector of reshaped normalized train dataset input 
  std::vector<float> y_train;                    ///< The vector of reshaped normalized train dataset ouput
  std::vector<float> x_test;                     ///< The vector of reshaped normalized test dataset ouput
  std::vector<float> y_test;                     ///< The vector of reshaped normalized test dataset ouput
};
