...
public:
  ...
  /**
  * @brief Constructor
  * @param [in] modelName The symbol of the stock is the neural network model name.
  ...
  */
  ...
  /**
  * @brief A callback method to customize writing to a file, the trained neural network output data.
  * @param [in] fileName The file name on which data needs to be written.
  * @param [in] data The neural network data that will be written to the given file.
  */
...
private:
  ...
  std::string companyName;                             ///< The name of the company whose model is been trained
  std::shared_ptr<StockLSTM> lstmNetwork;              ///< The pointer to StockLSTM neural network object 
  std::shared_ptr<torch::optim::Optimizer> optimizer;  ///< The pointer to Pytorch's Optimizer object
};
